...
NGSadmix -minMaf 0.02 -minInd $minind -likes $angsd_dir/$prefix.${sample_file}.beagle.gz -K $K -outfiles $ngsadmix_dir/$prefix.${sample_file}_k${K}/$prefix.${sample_file}_k${K}_s${seed} -P $nt -tol 0.000001 -seed $seed >& $ngsadmix_dir/$prefix.${sample_file}_k${K}/$prefix.${sample_file}_k${K}_s${seed}.err 
