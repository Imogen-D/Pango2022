...
#called from plot_locator_sbatch.sh
...
parser$add_argument('--error',default="FALSE",type="character",help="calculate error and plot summary? requires known locations for all samples. T / F. default = F")
...
longlat <- TRUE #args$longlat

load("/work/idumville/pangolins/RAD/data/cntrymap.Rdata")
...
pdf(paste0(out,"_windows.pdf"),width=width,height=height)
...
for(i in samples){
  ...
  if(usemap=="T"){
    plot(map,axes=T,cex.axis=0.5,tck=-0.03,border="white",
         xlim=c(min(na.omit(c(sample$xpred,sample$x)))-1, #was 6 for all
                max(na.omit(c(sample$xpred,sample$x)))+1),
         ylim=c(min(na.omit(c(sample$ypred,sample$y)))-1,
                max(na.omit(c(sample$ypred,sample$y)))+1),
         col="grey",lwd=0.35)
  } else {
  ...
  points(x=locs$x,y=locs$y,col="dodgerblue3",pch=1,cex=0.75,lwd=0.5)
  ...
  # }
  #pb$tick()
  print("getting the area of the contour")

clineskd <- contourLines(kd)
x1 = lapply(clineskd, function(l) l[[1]]) #getting level elements of clines

if(!is.na(levels[1])){
clinenum <- which(abs(as.numeric(x1)-as.numeric(levels[1])) == min(abs(as.numeric(x1) - as.numeric(levels[1]))))
x <- clineskd[[clinenum[1]]][["x"]]
y <- clineskd[[clinenum[1]]][["y"]]
A = 0.5* abs( sum( x[1:(length(x)-1)]*y[2:length(x)] - y[1:(length(x)-1)]*x[2:length(x)] ) )
write.table(paste(A, i, levels[1], prob[1], sep="\t"), file=paste0(out,"_areas.txt"), append=TRUE)
}

if(!is.na(levels[2])){
clinenum <- which(abs(as.numeric(x1)-as.numeric(levels[2])) == min(abs(as.numeric(x1) - as.numeric(levels[2]))))
x <- clineskd[[clinenum[1]]][["x"]]
y <- clineskd[[clinenum[1]]][["y"]]
A = 0.5* abs( sum( x[1:(length(x)-1)]*y[2:length(x)] - y[1:(length(x)-1)]*x[2:length(x)] ) )
write.table(paste(A, i, levels[2], prob[2], sep="\t"), file=paste0(out,"_areas.txt"), append=TRUE)
}

if(!is.na(levels[3])){
clinenum <- which(abs(as.numeric(x1)-as.numeric(levels[3])) == min(abs(as.numeric(x1) - as.numeric(levels[3]))))
x <- clineskd[[clinenum[1]]][["x"]]
y <- clineskd[[clinenum[1]]][["y"]]
A = 0.5* abs( sum( x[1:(length(x)-1)]*y[2:length(x)] - y[1:(length(x)-1)]*x[2:length(x)] ) )
write.table(paste(A, i, levels[3], prob[3], sep="\t"), file=paste0(out,"_areas.txt"), append=TRUE)
}

}
plot(1, type = "n", axes=FALSE, xlab="", ylab="")
legend(x="top",
       legend=c("Rural Training Localities","Locality of Origin","Predicted Locations"), #legend=c("Training Locations","Sample Location","Predicted Locations"),
       col=c("dodgerblue3","red3","black"),
       pch=16,cex=0.5,pt.cex=2,bty='n',horiz=T,x.intersp = 1)
dev.off()



#other method for calculating area hopefully spatial
#library(maptools)
lines <- ContourLines2SLDF(clineskd)
#my concern is the clineskd object has many levels (13) so which one am I choosing
#for example my levels are;  levels [1] 0.02401219 0.28351409 0.60905413
#but my lines$levels are;  [1] "0.05" "0.1"  "0.15" "0.2"  "0.25" "0.3"  "0.35" "0.4"  "0.45" "0.5" "0.55" "0.6"  "0.65" so which ones has contour() plot?



if(error=="T"){
  pdf(paste0(out,"_summary.pdf"),width=12,height=6.5,useDingbats = F)
  if(usemap=="T"){
    if(centroid_method=="gc"){
      ...
      map <- crop(map,c(min(na.omit(c(pd$xpred,pd$x)))-1, #used to be 10
                        max(na.omit(c(pd$xpred,pd$x)))+1,
                        min(na.omit(c(pd$ypred,pd$y)))-1,
                        max(na.omit(c(pd$ypred,pd$y)))+1))
      print(ggplot()+coord_map(projection = "mollweide",
                               xlim=c(min(na.omit(c(pd$xpred,pd$x)))-1, #used to be 10
                                      max(na.omit(c(pd$xpred,pd$x)))+1),
                               ylim=c(min(na.omit(c(pd$ypred,pd$y)))-1,
                                      max(na.omit(c(pd$ypred,pd$y)))+1))+
              ...
              scale_color_distiller(palette = "RdYlBu",name="Mean Distance\n(km)")+
      ...
      locsn <- ddply(locs,.(x,y),summarize,n=length(sampleID))
      truelocs <- merge(truelocs,locsn,c("x","y"))
      map <- crop(map,c(min(na.omit(c(pd$xpred,pd$x)))-1,
                        max(na.omit(c(pd$xpred,pd$x)))+1,
                        min(na.omit(c(pd$ypred,pd$y)))-1,
                        max(na.omit(c(pd$ypred,pd$y)))+1))
      print(ggplot()+coord_map(projection = "mollweide",
...
##very ugly way of getting all contour values TO BE TESTED ON NON-ITERATIVE, BECAUSE CURRENTLY PRINTING SAME SAMPLE OVER AND OVER
if(error!="F"){
print("contour values")
for(i in pd$sampleID){
  print(i)
  sample <- pd[pd$sampleID==i,]
  try({
    kd <- kde2d(sample$xpred,sample$ypred,n = 80,
...
###end of ugly way of getting contours
q(save="no")


############# Getting distance to closest market (as function of market size? ) #######

library(rgeos)
library(rgdal) #needed for projections
library(leaflet)
library(magrittr)
library(dplyr)
library(ggplot2)
...
allpred <- read.csv("/work/idumville/pangolins/RAD/results/locator/indvidualpredictionswgeo.txt", sep="\t", header=T)
allpred <- allpred[,c(1,2,3)]

alldata <- read.csv("/work/jsalmona/pangolins/RAD/data/RAD_pango_recap.txt", sep="\t", header=T)
alldata <- alldata[,c("Latitude", "Longitude", "Locality", "Market_Type", "lineage")] 
alldata <- alldata[which(alldata$Market_Type == "Rural_Market"),]
alldata <- alldata[which(alldata$lineage == "WCA"),]

alldata <- data.frame(alldata %>% group_by_all() %>% summarise(COUNT = n()))

plocs=as.matrix(allpred[,c("kd_x","kd_y")])
mlocs=as.matrix(alldata[,c("Longitude","Latitude")])
                               
dists <- spDists(as.matrix(plocs),as.matrix(mlocs),longlat=TRUE)

rownames(dists) <- allpred$sampleID
colnames(dists) <- alldata$Locality

alldists <- melt(dists)
names(alldists) <- c("sampleID", "Locality", "Distance")
alldists <- merge(alldists, alldata[,c("Locality", "COUNT")], by="Locality")


mindistances <- apply(dists, 1, FUN = min)
market <- alldata$Locality[c(apply(dists, 1, FUN = which.min))]
numbersamp <- alldata$COUNT[c(apply(dists, 1, FUN = which.min))]

df <- data.frame (sampleID  = allpred$sampleID,
                  distances = mindistances,
                  market = market,
                  nsamp = numbersamp
                  )

write.table(df, "/work/idumville/pangolins/RAD/results/locator/mindistancemm3.txt", quote=FALSE, row.names=FALSE)

#plotting
lm_eqn <- function(df){
    m <- lm(distances ~ nsamp, data=df);
...
png("/work/idumville/pangolins/RAD/results/locator/mindistance_per_num_samps.png", width = 750,height = 750)
ggplot(df, aes(y=distances, x=nsamp)) + geom_boxplot(aes(group=nsamp), outlier.shape=NA) + theme_bw() + labs(title="Distance to closest market over number of samples",
        x ="Number of Market Samples", y = "Distance (km)") + geom_point(aes(colour=market), size = 0.5) + geom_smooth(method = "lm", se=FALSE, color="black", formula = y ~ x, lwd=0.5) +
        geom_text(x = 5, y = 70, label = lm_eqn(df), parse = TRUE) + scale_color_manual(values = mycolors)
dev.off()

lm_eqn <- function(df){
    m <- lm(Distance ~ COUNT, data=df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 2),
              b = format(unname(coef(m)[2]), digits = 2),
             r2 = format(summary(m)$r.squared, digits = 3)))
...
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(29)

png("/work/idumville/pangolins/RAD/results/locator/alldistance_per_num_samps.png", width = 750,height = 750)
ggplot(alldists, aes(y=Distance, x=COUNT)) + geom_boxplot(aes(group=COUNT), outlier.shape = NA) + theme_bw() + geom_point(aes(colour=Locality), size = 0.5) +
  labs(title="Distance to all markets over number of samples", x ="Number of Market Samples", y = "Distance (km)") + geom_smooth(method = "lm", se=FALSE, color="black", formula = y ~ x, lwd=0.5) + geom_text(x = 10, y = 600, label = lm_eqn(alldists), parse = TRUE) + scale_colour_manual(values = mycolors)
